...
#Indexing the payload field "category" for efficient filtering. Use different index to create different types of indexes for different payload fields like "is_active" 
...
def search_with_filter(query, query_filter=None, top_k=3):
    ...
    results = client.query_points(
        ...
        limit=top_k,
        with_payload=True,
        query_filter=query_filter, #extra parameter to filter the results based on the provided filter
    ).points


    return results

##Filter Syntax - must, should, must_not 
...
question = "Does the company provide reimbursement?"
