...
#Part1 Coverts Job Description to structured JSON using pydantic
...
#Part2 Resume Schema and Parsing Schema

class MatchResult(BaseModel):  #scoring schema for matching job and resume
    score: float
    details: dict
...
def final_score(job,resume):  #extracts the final score and verdict based on job description and resume
...
def parse_resume(resume_text):  #parses the resume text and returns a structured Resume object
    system_prompt = f"""
...
#pdf and docx parser for extracting info from resume
...
def read_resume(file_path):  #read resume file based on extension
...
#Part3 - read all resumes from the folder and parse them and LLM call and store the results in a array
resume_folder = Path("resumes")
all_results=[]

for file_path in resume_folder.iterdir():
    #path of resume file that is to be parsed
    ...
    time.sleep(5) #timer for rate limiting and prevention against DOS attacks 
...
#sorting in descending order based on score and %
all_results.sort(
    key=lambda candidate: candidate["score"],
    reverse=True
)

#list of top 2 and worst 2 candidates based on score
top_2 = all_results[:2]
worst_2 = all_results[-2:]

#showing the top 2 and worst 2 candidates based on score
